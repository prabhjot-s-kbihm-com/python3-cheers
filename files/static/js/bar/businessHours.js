...
(function($) {
    $.fn.businessHours = function(opts) {
        var defaults = {
            ...
            defaultOperationTimeFrom: '19:00',
            defaultOperationTimeTill: '2:00',
            defaultActive: true,
            //labelOn: "Working day",
            //labelOff: "Day off",
            //labelTimeFrom: "from:",
                ...
                return {
                    serialize: function() {
                        ...
                        container.find(".operationState").each(function(num, item) {
                            ...
                            data.push({
                                day: dayContainer.find('.weekday').html(),
...
//            restoreWeekHours: function(){
//                alert("restoring values functionality in progress.");
//            },
            initView: function(options) {
                ...
                $("#select-day").click(function () {
                    if ($('input#select-day').prop('checked')){
                    // Get start,end time from first container.
                     var startTime = $('.operationDayTimeContainer:first input[name="startTime"]').val();
                     var endTime = $('.operationDayTimeContainer:first input[name="endTime"]').val()
                     $.each($('.operationDayTimeContainer'), function(index, item){
                         $(this).find('input[name="startTime"]').val(startTime);
                         $(this).find('input[name="endTime"]').val(endTime);
                     });
                    }else{
                        console.log();
//                        $this.restoreWeekHours();
                    }
                 });
